...
class Near:
    """A NEAR client holding one signer, a nonce cache, and a connection pool.

    Zero-argument construction discovers config from ``NEAR_NETWORK``,
    ``NEAR_RPC_URL``, ``NEAR_ACCOUNT_ID``, ``NEAR_PRIVATE_KEY``, falling back
    to ``~/.near-credentials`` when only an account is known::

        near = Near()  # env discovery
        near = Near(network="testnet", account_id="alice.testnet", private_key="ed25519:...")
        near = Near.from_file("alice.testnet", network="testnet")
    """
    ...
    def balance(self, account_id: str | None = None) -> Amount:
        """The account's liquid balance (defaults to the signer's account)."""
        return self.account(account_id).amount

    def account(self, account_id: str | None = None) -> AccountView:
        """Account state: balance, locked stake, storage, code hash."""
        target = _core.default_account_id(self.signer, account_id)
        result = self._transport.call("query", _core.account_params(target))
    ...
    def access_keys(self, account_id: str | None = None) -> list[KeyInfo]:
        """All access keys on the account."""
        target = _core.default_account_id(self.signer, account_id)
        result = self._transport.call("query", _core.access_key_list_params(target))
        return [KeyInfo.model_validate(entry) for entry in result.get("keys", [])]

    def access_key(self, account_id: str, public_key: str | PublicKey) -> AccessKeyView:
    ...
    def transaction_status(
        ...
        *,
        ...
        return TransactionResult.model_validate(self._transport.call("tx", params))
    ...
    def call(
        ...
        *,
        ...
        return self.send_transaction(contract_id, [action], wait_until=wait_until, signer=signer)
    ...
    def ft_metadata(self, token_id: str) -> FTMetadata:
        ...
        if token_id not in self._ft_meta:
            self._ft_meta[token_id] = FTMetadata.model_validate(self.view(token_id, "ft_metadata"))
        return self._ft_meta[token_id]

    def ft_balance(self, token_id: str, account_id: str | None = None) -> TokenAmount:
        """The account's token balance (defaults to the signer's account)."""
        metadata = self.ft_metadata(token_id)
        target = _core.default_account_id(self.signer, account_id)
        raw = self.view(token_id, "ft_balance_of", {"account_id": target})
    ...
    def _registration_actions(
        ...
        if (
            not register
            or self.view(token_id, "storage_balance_of", {"account_id": receiver_id}) is not None
        ):
            return []
        bounds = self.view(token_id, "storage_balance_bounds")
    ...
    def ft_transfer(
        ...
        *,
        ...
        """
        value = as_token_amount(amount, self.ft_metadata(token_id))
        actions = self._registration_actions(token_id, receiver_id, register)
        actions.append(ft_transfer_action(receiver_id, value, memo))
        return self.send_transaction(token_id, actions, wait_until=wait_until)
    ...
    def sign_message(
        ...
        *,
        ...
        """Sign an off-chain NEP-413 message (no gas, nothing touches the chain)."""
        return _nep413_sign(
    ...
    def __repr__(self) -> str:
        ...
        return f"Near(network={self.network!r}, signer={who!r})"
